    ...
    def update(self, walls):
        ...
        for i, ship in enumerate(self.ships[:]):
            ...
            if ship.hp <= 0:  # Entferne das Schiff, wenn HP 0 sind
                ...
                if is_enemy:  # Wenn ein Feind stirbt, verdopple die Anzahl der Feinde
                    for _ in range(2):  # Zwei neue Feinde hinzufügen
        ...
        for bullet in self.bullets:
            ...
            for i, ship in enumerate(self.ships):
                if i != bullet.owner and self._collides(bullet, ship):  # Avoid friendly fire
                    ...
                    hit = True
                    if ship.hp <= 0 and i == 1:  # Punkte für das Töten eines Feindes
                        self.score[0] += 10  # Spieler erhält 10 Punkte
...
def generate_coins(num_coins, walls):
    ...
    for _ in range(num_coins):
        x, y = generate_valid_position(walls, WORLD_WIDTH, WORLD_HEIGHT)
        coins.append(pygame.Rect(x - 5, y - 5, 10, 10))  # Münze als kleines Rechteck
    return coins
...
def main():
    ...
    coins = generate_coins(20, walls)  # 20 Münzen auf dem Spielfeld
    ...
    while running:
        ...
        for coin in coins[:]:
            ...
            pygame.draw.rect(screen, (255, 215, 0), coin_screen)  # Gelbe Münzen

            # Prüfe, ob der Spieler eine Münze einsammelt
            player_rect = pygame.Rect(player.x - 10, player.y - 10, 20, 20)
            if player_rect.colliderect(coin):
                coins.remove(coin)  # Münze entfernen
                engine.score[0] += 1  # Spieler erhält 1 Punkt
