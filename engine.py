...
# Generate a random position for the ship within the labyrinth
# The position is generated by checking if it is valid (not colliding with walls).
def generate_valid_position(walls, world_width, world_height, max_retries=1000):
    for _ in range(max_retries):
        x = random.randint(100, world_width - 100)
        y = random.randint(100, world_height - 100)
        if is_position_valid(x, y, walls):
            return x, y
    raise RuntimeError("Unable to generate a valid position")
...
def show_start_menu(screen):
    ...
    title_text = font.render("Choose Mode", True, (0, 0, 0))
    player_text = font.render("Player - Press 1", True, (0, 0, 0))
    agent_text = font.render("Dummy Agent - Press 2", True, (0, 0, 0))

    while True:
        screen.fill(WHITE)
        screen.blit(title_text, (SCREEN_WIDTH // 2 - title_text.get_width() // 2, 100))
        screen.blit(player_text, (SCREEN_WIDTH // 2 - player_text.get_width() // 2, 200))
        screen.blit(agent_text, (SCREEN_WIDTH // 2 - agent_text.get_width() // 2, 300))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1:  # Player mode
                    return "player"
                if event.key == pygame.K_2:  # Dummy Agent mode
                    return "agent"
...
def main():
    ...
    # Show the start menu and get the user's choice
    mode = show_start_menu(screen)

    # Initialize the dummy agent if chosen
    agent = DummyAgent(ship_index=0) if mode == "agent" else None
    ...
    while running:
        ...
        # Player controls (only if the player is playing)
        if mode == "player":
        ...
        # Dummy agent controls the player ship if in agent mode
        if mode == "agent":
            wall_dicts = [{"x": wall.x, "y": wall.y, "width": wall.width, "height": wall.height} for wall in walls]
            actions = agent.decide(engine.get_game_state(), wall_dicts)
        ...
        for i, ship in enumerate(engine.ships):
            # Draw the ship
            color = BLUE if i == 0 else RED  # Blue for player, Red for enemy
        ...
        # Draw the laser for the dummy agent
        if mode == "agent":
            my_ship = engine.ships[0]
            rad = math.radians(my_ship.angle)
            laser_end_x = my_ship.x + math.cos(rad) * 1000  # Laser length
        ...
        screen.blit(hp_text, (20, 20))  # Slightly offset from the rectangle
        screen.blit(score_text, (20, 40))  # Slightly offset below the HP text
